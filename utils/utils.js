...
async function downloadTexture(device, texture, filename = "image.png") {
  ...
  const copiedBuffer = createGPUBuffer(
    device,
    new Uint8Array(bufferWidth * texture.height * 4),
  ...
  commandEncoder.copyTextureToBuffer(
    ...
    { buffer: copiedBuffer, bytesPerRow: bufferWidth * 4 },
  ...
  await copiedBuffer.mapAsync(
    GPUMapMode.READ,
    0,
    bufferWidth * texture.height * 4
  );
...
async function downloadStencilTexture(device, texture, filename = "image.png") {
  ...
  const copiedBuffer = createGPUBuffer(
    device,
    new Uint8Array(bufferWidth * texture.height),
    GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ
  );

  const commandEncoder = device.createCommandEncoder();
  commandEncoder.copyTextureToBuffer(
    { texture: texture, origin: { x: 0, y: 0 }, aspect: "stencil-only" },
...
async function dumpTextureF32(device, texture) {
  ...
  const copiedBuffer = createGPUBuffer(
    device,
    new Float32Array(bufferWidth * texture.height),
    GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ
  );
